...
import { CardContent, Typography, Chip, CardActionArea } from '@mui/material';
  ...
  const handleEventClick = event => {
    ...
    if (serviceRequestStatus === 'PENDING' && isServiceProvider) {
      ...
      notificationManager.warning(
        'Waiting for service provider to accept the service request.. Please check back later.',
        'Pending... '
      );
    }
  ...
  return (
    <>
        ...
        eventContent={eventInfo => (
          <CardActionArea>
            <CardContent>
              ...
              {/*   Sent by: */}
              {/* </Typography> */}
              <Chip color="primary" label={get(eventInfo, ['event', '_def', 'extendedProps', 'status'])} />
            </CardContent>
          </CardActionArea>
        )}
      />
      ...
      {shouldShowStatusChangeDialog && (
        <ServiceRequestStatusDialog handleClose={handleCloseStatusChangeDialog} service_request={selectedEvent} />
      )}
    </>
  );
};
