...
export const ServiceRequestDetail = ({ selectedServiceRequestRef, handleClose }) => {
    ...
    useEffect(() => {
        if (get(user, 'ref') === get(selectedServiceRequest, 'service_provider_ref') && selectedServiceRequestStatus === serviceRequestStatusOptions.pending) {
    ...
    console.debug(selectedServiceRequestStatus === serviceRequestStatusOptions.pending);
    const actions = [
        {
